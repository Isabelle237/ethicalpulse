            ...
            if form.is_valid():
                ...
                if scan.is_active:
                    run_scheduled_scan.apply_async(
                        args=[scan.id], eta=scan.next_run_time
                    )
                messages.success(request, "Scan planifié mis à jour avec succès")
                return redirect("scan_list")
            else:
                messages.error(
                    request, "Erreur lors de la mise à jour du scan planifié"
                )
        else:
...
def scheduled_scans(request):
    ...
    scheduled_scans = ScheduledScan.objects.filter(created_by=request.user).order_by(
        "next_run_time"
    )
        ...
        data = [
            {
                ...
                "name": scan.name,
                "next_run_time": (
                    scan.next_run_time.isoformat() if scan.next_run_time else None
                ),
                "frequency": scan.frequency,
                "is_active": scan.is_active,
                "tool": scan.tool.name,
