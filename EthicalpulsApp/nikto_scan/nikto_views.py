...
def scan_result_detail(request, scan_id):
    ...
    return render(request, "admin/scan_detail.html", {"scan": scan, "result": result})
...
def handle_nikto_scan(project, option, request):
    ...
    try:
        ...
        valid_options = [
            opt[0] for opt in NiktoResult._meta.get_field("option").choices
        ]
        ...
        return (
            True,
            f"Scan Nikto lancé pour le projet '{project.name}' avec l'option '{option}'",
        )
