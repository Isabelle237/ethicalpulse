...
def build_nmap_command(target, option=None):
    ...
    if needs_root and not is_root_user():
        logger.warning(
            "Options Nmap nécessitent les privilèges root, mais le processus n'est pas root. Tentative d'exécution via sudo."
        )
        cmd.append(
            "sudo"
        )  # nécessite config sudoers sans mot de passe pour celery_user
