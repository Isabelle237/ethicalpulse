...
def handle_netcat_scan(project, option, target_port, request):
    ...
    try:
        ...
        valid_options = [
            opt[0] for opt in NetcatResult._meta.get_field("option").choices
        ]
        ...
        return (
            True,
            f"Scan Netcat lancé pour le projet '{project.name}' avec l'option '{option}'",
        )
