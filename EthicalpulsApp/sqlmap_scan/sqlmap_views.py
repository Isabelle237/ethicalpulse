...
def handle_sqlmap_scan(request):
    ...
    try:
        ...
        logger.info(
            f"[SQLMAP] Scan #{scan.id} lancé pour {project.url} avec options : {options_list}"
        )
        ...
        logger.error(
            f"[SQLMAP] Échec lancement du scan #{scan.id} : {str(e)}", exc_info=True
        )
...
def sqlmap_report_pdf(request, scan_id):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="sqlmap_report_scan_{scan.id}.pdf"'
    )
...
def download_sqlmap_report(request, scan_id):
    ...
    buffer.seek(0)
    return FileResponse(
        buffer, as_attachment=True, filename=f"rapport_sqlmap_{scan.id}.pdf"
    )
    scan = get_object_or_404(Scan, id=scan_id, tool__iexact="sqlmap")
    result = scan.sqlmapresults.first()
    if not result:
